...
export function useSlider(slideCount: number, loop: boolean) {
    ...
    const scrollToPrevious = useCallback<SliderContextObject['scrollToPrevious']>(() => {
        const container = containerRef.current;
        if (!container) {
            return;
        }
...
const SliderItemsContext = createContext<React.RefObject<HTMLElement | null> | null>(
    null,
);
...
export function SliderItems<TAs extends React.ElementType = 'div'>({
    ...
    return (
        <Component ref={ref as any} {...restProps} onScroll={onScroll}>
            {children}
        </Component>
    );
};
...
export function SliderItem<TAs extends React.ElementType = 'div'>({
    ...
    return (
        <Component ref={ref as any} data-active={index === currentSlide} {...restProps}>
            {children}
        </Component>
    );
};
...
export function SliderCount<TAs extends React.ElementType = 'span'>({
    ...
    padStart = 1,
    ...restProps
}: SliderCountProps<TAs> & Omit<React.ComponentPropsWithoutRef<TAs>, 'children'>) {
    const Component = asProp || 'span';
    const { slideCount, currentSlide } = useContext(SliderContext);

    return (
        <Component {...restProps}>
            {String(slideCount === 0 ? slideCount : currentSlide + 1).padStart(padStart, '0')}
        </Component>
    );
};
...
export function SliderMax<TAs extends React.ElementType = 'span'>({
    ...
    prefix,
    ...restProps
}: SliderMaxProps<TAs> & Omit<React.ComponentPropsWithoutRef<TAs>, 'children'>) {
    const Component = asProp || 'span';
...
export function SliderPrev<TAs extends React.ElementType = 'button'>({
    ...
    const { prev, loop, scrollPosition, scrollToPrevious } = useContext(SliderContext);
...
export function SliderNext<TAs extends React.ElementType = 'button'>({
    ...
    const Component = asProp || 'button';
    const { next, loop, scrollPosition, scrollToNext } = useContext(SliderContext);
